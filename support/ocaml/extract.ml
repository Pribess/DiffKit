...
let expression iterator expression =
  ...
  | Texp_apply ({ exp_desc = Texp_ident (path, _, _); _ }, _) ->
      emit "direct" (Path.name path) expression.exp_loc
  | Texp_apply _ -> emit "indirect" "" expression.exp_loc
  | Texp_send _ -> emit "indirect" "object-method" expression.exp_loc
  | _ -> ());
